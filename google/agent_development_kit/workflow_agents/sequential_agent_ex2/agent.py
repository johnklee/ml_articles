...
def decide_initial_topic(topic: str, tool_context: ToolContext):
  """Decides the initial topic for further writing.

  Args:
    topic: Initial topic to start writing.

  Returns:
    dict with key `topic` to hold the writing topic.
  """
...
- Call tool `decide_initial_topic` to decide the initial topic if user does not
  give one.

Output *only* the story/document text. Do not add introductions or explanations."""),
    description="Writes the initial document draft based on the topic, aiming for some initial substance.",
    output_key=STATE_CURRENT_DOC,
    tools=[decide_initial_topic, exit_loop])
